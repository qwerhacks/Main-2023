<script lang="ts">
	...
	import { bringToTop } from '../utils/Draggable.svelte';

	const tabs = ['Sponsor', 'Donate', 'Volunteer'];

	let activeTab = 0;

	let shouldShowBenevityInfo = false;
	...
	let url = '';
	let postItName: string = 'BenevityPostIt';
...
<div class="relative">
	...
	>
		<div
			slot="header"
			class="mt-8 flex flex-row px-2 sm:px-6 md:px-12 gap-4 sm:gap-6 md:gap-8 md:max-w-[80%]"
		>
			{#each tabs as tab, i}
				<button
		...
		<div class="flex-row items-center gap-4 hidden md:flex">
			...
			<div
				class="flex-grow border-4 border-[#200B3A] bg-white w-32 p-1 px-4 text-lg font-jetbrains"
			>
				{url.toString()}
			</div>
		</div>
